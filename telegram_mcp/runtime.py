...
# Whisper model for local voice transcription (mlx-whisper, Apple Silicon). [fork]
...
def render_text_entities(text: str, entities) -> str:
    ...
    *invisible* entities rather than literal text.
    ...
    Markdown `[visible text](url)`. Pure style
...
def get_message_text(message) -> str:
    ...
    falling back to `rich_message_text` (block-format post) when the plain
    text is empty."""
    ...
    if text:
        ...
        text = rich_message_text(message)
