...
async def transcribe_voice_message(
    ...
    """
    Transcribe a voice or audio message to text using a LOCAL Whisper model
    (mlx-whisper, on this machine) — no audio ever leaves the server, no API
    key or network call required. Downloads and transcribes automatically, no
    need to download the media separately first.

    [fork] This is the default/preferred transcription tool on this server.
    A separate tool, transcribe_voice, also exists (upstream) and routes audio
    to a third-party API (Groq) or Telegram's own Premium transcription
    instead — only use that one if this local tool is unavailable/fails, or a
    caller explicitly asks for one of its two engines by name.
    ...
    try:
        ...
        try:
            try:
                ...
                return log_and_format_error(
                    "transcribe_voice_message", dl_err, chat_id=chat_id, message_id=message_id
                )
